...
for FILE_NAME in f_books f_fb f_lognormal f_normal f_udense f_usparse f_wiki f_osm; do
    for TIME_WINDOW in 100000 1000000 10000000; do
        echo "DALindex (TUNE): $FILE_NAME, $TIME_WINDOW";
        ...
        echo "#define TIME_WINDOW $TIME_WINDOW" >> parameters.hpp;
        ...
        g++ benchmark/run_DALindex.cpp -std=c++17 -march=native -fopenmp -O3 -w -o z_run_test && ./z_run_test >> benchmark/exp_window_size.txt                     
    done
done

for FILE_NAME in f_books f_fb f_lognormal f_normal f_udense f_usparse f_wiki f_osm; do
    for TIME_WINDOW in 100000 1000000 10000000 ; do
        echo "ALEX: $FILE_NAME, $TIME_WINDOW";
        bash utils/reset_default_config.sh;
        > parameters.hpp;
        echo "#pragma once" >> parameters.hpp;
        echo "#define DATA_DIR \"$DIR\"" >> parameters.hpp;
        ...
        echo "#define CPU_CLOCK 3400000000" >> parameters.hpp;
        echo "#define RUNFLAG 0" >> parameters.hpp;
        g++ benchmark/run_alex.cpp -std=c++17 -march=native -fopenmp -O3 -w -o z_run_test && ./z_run_test >> benchmark/exp_window_size.txt
    done
done
...
for FILE_NAME in f_books f_fb f_lognormal f_normal f_udense f_usparse f_wiki f_osm; do
    for TIME_WINDOW in 100000 1000000 10000000 ; do
        for FANOUT in 16 64 256 1024; do
            echo "PGM + B+Tree: $FILE_NAME, $FANOUT, $TIME_WINDOW";
            ...
            echo "#define CPU_CLOCK 3400000000" >> parameters.hpp;
            echo "#define RUNFLAG 0" >> parameters.hpp;
            g++ benchmark/run_pgm.cpp -std=c++17 -march=native -fopenmp -O3 -w -o z_run_test && ./z_run_test >> benchmark/exp_window_size.txt
            g++ benchmark/run_btree.cpp -std=c++17 -march=native -fopenmp -O3 -w -o z_run_test && ./z_run_test >> benchmark/exp_window_size.txt                        
        done
    done
done
...
for FILE_NAME in f_books f_fb f_lognormal f_normal f_udense f_usparse f_wiki f_osm; do
    for TIME_WINDOW in 100000 1000000 10000000 ; do
        echo "CARMI + IMTree: $FILE_NAME, $TIME_WINDOW";
        bash utils/reset_default_config.sh;
        > parameters.hpp;
        echo "#pragma once" >> parameters.hpp;
        echo "#define DATA_DIR \"$DIR\"" >> parameters.hpp;
        ...
        echo "#define CPU_CLOCK 3400000000" >> parameters.hpp;
        echo "#define RUNFLAG 0" >> parameters.hpp;
        # g++ benchmark/run_carmi.cpp -std=c++17 -march=native -fopenmp -O3 -w -o z_run_test && ./z_run_test >> benchmark/exp_window_size.txt
        g++ benchmark/run_imtree.cpp -std=c++17 -march=native -fopenmp -O3 -w -o z_run_test && ./z_run_test >> benchmark/exp_window_size.txt
    done
done