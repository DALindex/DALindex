...
for FILE_NAME in f_books f_fb f_lognormal f_normal f_udense f_usparse f_wiki f_osm; do
    for UPDATE_LENGTH in 2 4 8; do
        echo "DALindex (TUNE): $FILE_NAME, $UPDATE_LENGTH";
        ...
        echo "#define TEST_LEN TIME_WINDOW * $UPDATE_LENGTH" >> parameters.hpp;
        ...
        g++ benchmark/run_DALindex.cpp -std=c++17 -march=native -fopenmp -O3 -w -o z_run_test && ./z_run_test >> benchmark/exp_update_length.txt                     
    done
done

for FILE_NAME in f_books f_fb f_lognormal f_normal f_udense f_usparse f_wiki f_osm; do
    for UPDATE_LENGTH in 2 4 8; do
        echo "ALEX: $FILE_NAME, $UPDATE_LENGTH";
        bash utils/reset_default_config.sh;
        > parameters.hpp;
        echo "#pragma once" >> parameters.hpp;
        echo "#define DATA_DIR \"$DIR\"" >> parameters.hpp;
        ...
        echo "#define CPU_CLOCK 3400000000" >> parameters.hpp;
        echo "#define RUNFLAG 0" >> parameters.hpp;
        g++ benchmark/run_alex.cpp -std=c++17 -march=native -fopenmp -O3 -w -o z_run_test && ./z_run_test >> benchmark/exp_update_length.txt
    done
done
...
for FILE_NAME in f_books f_fb f_lognormal f_normal f_udense f_usparse f_wiki f_osm; do
    for UPDATE_LENGTH in 2 4 8; do
        for FANOUT in 16 64 256 1024; do
            echo "PGM + B+Tree: $FILE_NAME, $FANOUT, $UPDATE_LENGTH";
            ...
            echo "#define CPU_CLOCK 3400000000" >> parameters.hpp;
            echo "#define RUNFLAG 0" >> parameters.hpp;
            g++ benchmark/run_pgm.cpp -std=c++17 -march=native -fopenmp -O3 -w -o z_run_test && ./z_run_test >> benchmark/exp_update_length.txt
            g++ benchmark/run_btree.cpp -std=c++17 -march=native -fopenmp -O3 -w -o z_run_test && ./z_run_test >> benchmark/exp_update_length.txt                        
        done
    done
done
...
for FILE_NAME in f_books f_fb f_lognormal f_normal f_udense f_usparse f_wiki f_osm; do
    for UPDATE_LENGTH in 2 4 8; do
        echo "CARMI + IMTree: $FILE_NAME, $UPDATE_LENGTH";
        bash utils/reset_default_config.sh;
        > parameters.hpp;
        echo "#pragma once" >> parameters.hpp;
        echo "#define DATA_DIR \"$DIR\"" >> parameters.hpp;
        ...
        echo "#define CPU_CLOCK 3400000000" >> parameters.hpp;
        echo "#define RUNFLAG 0" >> parameters.hpp;
        # g++ benchmark/run_carmi.cpp -std=c++17 -march=native -fopenmp -O3 -w -o z_run_test && ./z_run_test >> benchmark/exp_update_length.txt
        g++ benchmark/run_imtree.cpp -std=c++17 -march=native -fopenmp -O3 -w -o z_run_test && ./z_run_test >> benchmark/exp_update_length.txt
    done
done