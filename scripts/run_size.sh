...
for FILE_NAME_R in f_books f_fb f_lognormal f_normal f_udense f_usparse f_wiki f_osm; do
    echo "DALindex (TUNE): $FILE_NAME_R";
    ...
    echo "#define TIME_WINDOW 100000" >> parameters.hpp;
    ...
    g++ benchmark/run_size.cpp -std=c++17 -march=native -fopenmp -O3 -w -o z_run_test && ./z_run_test >> benchmark/exp_size.txt                     
done

for FILE_NAME_R in f_books f_fb f_lognormal f_normal f_udense f_usparse f_wiki f_osm; do
    echo "IMTree: $FILE_NAME_R";
    bash utils/reset_default_config.sh;
    > parameters.hpp;
    echo "#pragma once" >> parameters.hpp;
    echo "#define DATA_DIR \"$DIR\"" >> parameters.hpp;
...
for FILE_NAME_R in f_books f_fb f_lognormal f_normal f_udense f_usparse f_wiki f_osm; do
    for FANOUT in 16 64 256 1024; do
        ...
        echo "#define SEED 1" >> parameters.hpp;
        echo "#define CPU_CLOCK 3400000000" >> parameters.hpp;
        echo "#define RUNFLAG 5" >> parameters.hpp;
        g++ benchmark/run_size.cpp -std=c++17 -march=native -fopenmp -O3 -w -o z_run_test && ./z_run_test >> benchmark/exp_size.txt 
    done
done
...
for FILE_NAME_R in f_books f_fb f_lognormal f_normal f_udense f_usparse f_wiki f_osm; do
    for FANOUT in 16 64 256 1024; do
        ...
        echo "#define SEED 1" >> parameters.hpp;
        echo "#define CPU_CLOCK 3400000000" >> parameters.hpp;
        echo "#define RUNFLAG 4" >> parameters.hpp;
        g++ benchmark/run_size.cpp -std=c++17 -march=native -fopenmp -O3 -w -o z_run_test && ./z_run_test >> benchmark/exp_size.txt 
    done
done