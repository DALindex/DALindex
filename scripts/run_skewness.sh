...
for FILE_NAME in f_books f_fb f_lognormal f_normal f_udense f_usparse f_wiki f_osm; do
    for NO_STD in 1 2 3 4; do
        echo "DALindex (TUNE): $FILE_NAME, $NO_STD";
        ...
        echo "#define TIME_WINDOW 5000000" >> parameters.hpp;
        echo "#define MATCH_RATE 1000" >> parameters.hpp;
        echo "#define TEST_LEN 80000000" >> parameters.hpp;
        ...
        echo "#define NO_STD $NO_STD" >> parameters.hpp;
        g++ benchmark/run_skewness.cpp -std=c++17 -march=native -fopenmp -O3 -w -o z_run_test && ./z_run_test >> benchmark/exp_skewness.txt                     
    done
done

for FILE_NAME in f_books f_fb f_lognormal f_normal f_udense f_usparse f_wiki f_osm; do
    for NO_STD in 1 2 3 4; do
        echo "ALEX: $FILE_NAME, $NO_STD";
        bash utils/reset_default_config.sh;
        > parameters.hpp;
        echo "#pragma once" >> parameters.hpp;
        echo "#define DATA_DIR \"$DIR\"" >> parameters.hpp;
...
for FILE_NAME in f_books f_fb f_lognormal f_normal f_udense f_usparse f_wiki f_osm; do
    for NO_STD in 1 2 3 4; do
        for FANOUT in 16 64 256 1024; do
            echo "PGM + B+Tree: $FILE_NAME, $FANOUT, $NO_STD";
...
for FILE_NAME in f_books f_fb f_lognormal f_normal f_udense f_usparse f_wiki f_osm; do
    for NO_STD in 1 2 3 4; do
        echo "CARMI + IMTree: $FILE_NAME, $NO_STD";
        bash utils/reset_default_config.sh;
        > parameters.hpp;
        echo "#pragma once" >> parameters.hpp;
        echo "#define DATA_DIR \"$DIR\"" >> parameters.hpp;
        ...
        echo "#define SEED 1" >> parameters.hpp;
        echo "#define CPU_CLOCK 3400000000" >> parameters.hpp;
        echo "#define RUNFLAG 3" >> parameters.hpp;
        echo "#define NO_STD $NO_STD" >> parameters.hpp;
        g++ benchmark/run_skewness.cpp -std=c++17 -march=native -fopenmp -O3 -w -o z_run_test && ./z_run_test >> benchmark/exp_skewness.txt
    done
done