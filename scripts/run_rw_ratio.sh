...
for FILE_NAME in f_books f_fb f_lognormal f_normal f_udense f_usparse f_wiki f_osm; do
    for RW_RATIO in 1 10 100; do
        echo "DALindex (TUNE): $FILE_NAME, $RW_RATIO";
        ...
        echo "#define TEST_LEN 40800000" >> parameters.hpp;
        ...
        echo "#define RW_RATIO $RW_RATIO" >> parameters.hpp;
        g++ benchmark/run_rw_ratio.cpp -std=c++17 -march=native -fopenmp -O3 -w -o z_run_test && ./z_run_test >> benchmark/exp_rw_ratio.txt                     
    done
done

for FILE_NAME in f_books f_fb f_lognormal f_normal f_udense f_usparse f_wiki f_osm; do
    for RW_RATIO in 1 10 100; do
        echo "ALEX: $FILE_NAME, $RW_RATIO";
        bash utils/reset_default_config.sh;
        > parameters.hpp;
        echo "#pragma once" >> parameters.hpp;
        echo "#define DATA_DIR \"$DIR\"" >> parameters.hpp;
...
for FILE_NAME in f_books f_fb f_lognormal f_normal f_udense f_usparse f_wiki f_osm; do
    for RW_RATIO in 1 10 100; do
        for FANOUT in 16 64 256 1024; do
            echo "PGM + B+Tree: $FILE_NAME, $FANOUT, $RW_RATIO";
...
for FILE_NAME_R in f_books f_fb f_lognormal f_normal f_udense f_usparse f_wiki f_osm; do
    for RW_RATIO in 1 10 100; do
        echo "CARMI + IMTree: $FILE_NAME_R, $RW_RATIO";
        bash utils/reset_default_config.sh;
        > parameters.hpp;
        echo "#pragma once" >> parameters.hpp;
        echo "#define DATA_DIR \"$DIR\"" >> parameters.hpp;
