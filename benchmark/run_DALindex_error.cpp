...
#include "../src/DALroot_Self_Analysis.hpp"
...
void run_dalindex(vector<tuple<uint64_t, uint64_t, uint64_t>> & data)
{
    ...
    rootErrorRight.push_back(DALindex.get_right_root_error());
    rootErrorLeft.push_back(DALindex.get_left_root_error());
    segErrorRight.push_back(DALindex.get_avg_right_seg_error());
    segErrorLeft.push_back(DALindex.get_avg_left_seg_error());
    ...
    for(uint64_t i = startTime; i < maxTimestamp; i++ )
    {
        ...
        int tempRightError = DALindex.get_right_root_error();
        int tempLeftError = DALindex.get_left_root_error();
