...
void run_dalindex(vector<tuple<uint64_t, uint64_t, uint64_t>> & data)
{
    ...
    uint64_t initialSize = DALindex.get_total_size_in_bytes();
    ...
    for(uint64_t i = startTime; i < maxTimestamp; i++ )
    {
        ...
        while(get<1>(*it) == i)
        {
            ...
            DALindex.tree_range_search(searchTuple,tempJoinResult);

            DALindex.tree_insert(insertTuple);

            totalSize += DALindex.get_total_size_in_bytes();
    ...
    uint64_t finalSize = DALindex.get_total_size_in_bytes();
    ...
    #endif
    cout << ";Data=" << FILE_NAME_R << ";MatchRate=" << MATCH_RATE;
    ...
    cout << ";FinalSize=" << finalSize << ";FinalSizeExternal=" << finalSize << ";";
    cout << endl;
}
...
void run_alex(vector<tuple<uint64_t, uint64_t, uint64_t>> & data)
{
    ...
    uint64_t initialSize = alex_get_total_size_in_bytes(alex);
    uint64_t initialSizeExternal = alex_get_total_size_in_bytes(alex) + size_of_map();
    ...
    for(uint64_t i = startTime; i < maxTimestamp; i++ )
    {
        ...
        while(get<1>(*it) == i)
        {
            ...
            totalSize += alex_get_total_size_in_bytes(alex);
            totalSizeExternal += alex_get_total_size_in_bytes(alex) + size_of_map();
    ...
    uint64_t finalSize = alex_get_total_size_in_bytes(alex);
    uint64_t finalSizeExternal = alex_get_total_size_in_bytes(alex) + size_of_map();
    ...
    cout << ";FinalSize=" << finalSize << ";FinalSizeExternal=" << finalSizeExternal << ";";
    cout << endl;
}
...
void run_pgm(vector<tuple<uint64_t, uint64_t, uint64_t>> & data)
{
    ...
    uint64_t initialSize = pgm_get_total_size_in_bytes(pgm);
    uint64_t initialSizeExternal = pgm_get_total_size_in_bytes(pgm) + size_of_map();
    ...
    for(uint64_t i = startTime; i < maxTimestamp; i++ )
    {
        ...
        while(get<1>(*it) == i)
        {
            ...
            totalSize += pgm_get_total_size_in_bytes(pgm);
            totalSizeExternal += pgm_get_total_size_in_bytes(pgm) + size_of_map();
    ...
    uint64_t finalSize = pgm_get_total_size_in_bytes(pgm);
    uint64_t finalSizeExternal = pgm_get_total_size_in_bytes(pgm) + size_of_map();
...
void run_btree(vector<tuple<uint64_t, uint64_t, uint64_t>> & data)
{
    ...
    uint64_t initialSize = bt_get_total_size_in_bytes(btree);
    uint64_t initialSizeExternal = bt_get_total_size_in_bytes(btree) + size_of_map();
    ...
    for(uint64_t i = startTime; i < maxTimestamp; i++ )
    {
        ...
        while(get<1>(*it) == i)
        {
            ...
            totalSize += bt_get_total_size_in_bytes(btree);
            totalSizeExternal += bt_get_total_size_in_bytes(btree) + size_of_map();
    ...
    uint64_t finalSize = bt_get_total_size_in_bytes(btree);
    uint64_t finalSizeExternal = bt_get_total_size_in_bytes(btree) + size_of_map();
...
void run_imtree(vector<tuple<uint64_t, uint64_t, uint64_t>> & data)
{
    ...
    cout << "Algorithm=IMTree" << ";Data=" << FILE_NAME_R << ";MatchRate=" << MATCH_RATE;
    cout << ";Fanout=" << 0 << ";SplitError=" << 0 << ";TimeWindow=" << TIME_WINDOW << ";UpdateLength=" << (TEST_LEN/TIME_WINDOW)-1;
    cout << ";InitialSize=" << initialSize << ";InitialSizeExternal=" << initialSize;
    cout << ";AvgSize=" << (double)totalSize/loopCounter << ";AvgSizeExternal=" << (double)totalSize/loopCounter;
    cout << ";FinalSize=" << finalSize << ";FinalSizeExternal=" << finalSize << ";";
